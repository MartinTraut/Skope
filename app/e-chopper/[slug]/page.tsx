...
export function generateStaticParams() {
  return listByCategory("chopper").map((item) => ({ slug: item.id }));
}
...
export async function generateMetadata({
  ...
  if (!item || item.category !== "chopper") return {};
...
export default async function Page({
  ...
  if (!item || item.category !== "chopper") notFound();
