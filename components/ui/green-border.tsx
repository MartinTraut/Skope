...
/**
 * Pulsierender Rand als Shader — Quelle: 21st.dev Shader Builder („Pulsing
 * Border"), abgeleitet von Paper Shaders (Apache-2.0),
 * https://shaders.paper.design/pulsing-border
 *
 * Ohne Abhängigkeit: ein WebGL-Canvas, das seine Elternfläche füllt. Es
 * gehört hinter den Inhalt, nicht neben ihn:
 *
 ...
 *   </div>
 *
 * Zwei Dinge sind gegenüber der Vorlage geändert, beide nicht kosmetisch:
 *
 * 1. Die Palette. Der Builder liefert fünf verschiedene Grüntöne plus eine
 *    Farbtondrehung von 30 Grad — auf der Seite wären das fünf Grüns, die
 *    keiner Marke gehören. Hier laufen nur Schwarz, das eine Neon #9ef605
 *    und zwei Abstufungen desselben Tons; die Drehung steht auf 0.
 * 2. Reduzierte Bewegung. Die Vorlage läuft immer. Wer
 *    `prefers-reduced-motion` gesetzt hat, bekommt ein stehendes Bild und
 *    keinen Zeiger-Effekt — gezeichnet wird genau einmal.
 */
...
// Der Rahmen folgt der Fläche, nicht der kürzeren Kante. Die Vorlage misst
// in einem Raum, der auf min(Breite, Höhe) normiert ist: Bei einer breiten
// Kachel bleibt das Rechteck dadurch schmal in der Mitte stehen und liest
// sich als grüner Streifen, nicht als Rahmen. Hier wird in uv gerechnet,
// also je Achse auf -1..1 – das Rechteck sitzt damit bei jedem
// Seitenverhältnis auf den Kanten. Der doppelte Maßstab dieses Raums steckt
// in der Dicke und im Abfall des inneren Scheins.
vec3 shade(vec2 uv, vec2 p, float t) {
  vec2 q = (uv - 0.5) * 2.0;
  vec2 box = vec2(0.955, 0.90);
  vec2 d = abs(q) - box;
  float outside = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
  float thickness = mix(0.018, 0.11, u_paramA) * 1.6;
  float edge = 1.0 - smoothstep(thickness * 0.35, thickness, abs(outside));
  float perimeter = atan(q.y * box.x, q.x * box.y) / 6.2831853 + 0.5;
  float pulse = 0.5 + 0.5 * sin(perimeter * (5.0 + u_intensity * 9.0) - t * 1.8);
  float trail = pow(pulse, mix(7.0, 2.0, u_intensity));
  float innerGlow = exp(-abs(outside) * 18.0) * 0.32;
...
/**
 * Palette: Tinte als Grund, darüber ausschließlich Abstufungen des einen
 * Neons. Der Builder-Standard hatte fünf eigenständige Grüntöne und eine
 * Farbtondrehung von 30 Grad — auf dieser Seite gibt es genau ein Grün.
 */
const UNIFORMS = {
  colors: [
    [0.031, 0.035, 0.043], // --color-ink   #08090b
    [0.62, 0.965, 0.02], //   --color-neon  #9ef605
    [0.776, 1.0, 0.369], //   --color-neon-300 #c6ff5e
    [0.31, 0.482, 0.01], //   Neon halbiert
    [0.075, 0.19, 0.02], //   Neon im Auslauf
    [0.075, 0.19, 0.02],
    [0.075, 0.19, 0.02],
    [0.075, 0.19, 0.02],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Reduzierte Bewegung: ein Bild, kein Zeiger-Effekt, keine Schleife.
    const still = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const timeScale = still ? 0 : UNIFORMS.timeScale;
    const cursorEnabled = UNIFORMS.cursorEnabled && !still;
    ...
    // `render` ist unten als const-Pfeilfunktion gebunden, nicht als
    // Funktionsdeklaration: Nur dort behält TypeScript die Null-Verengung von
    // `canvas` und `gl` aus den beiden frühen Rückgaben bei. Der Vorgriff hier
    // ist zulässig, weil `requestRender` erst nach der Bindung läuft.
